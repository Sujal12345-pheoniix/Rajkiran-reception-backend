    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const visit = await tx.visit.create({
        data: {
          ...
          ipd_admission_date: visitData.visit.ipd_admission_date ? new Date(visitData.visit.ipd_admission_date) : null,
          ipd_discharge_date: visitData.visit.ipd_discharge_date ? new Date(visitData.visit.ipd_discharge_date) : null,
          ipd_stay_days: visitData.visit.ipd_stay_days ?? null,
          ipd_daily_expense: visitData.visit.ipd_daily_expense ?? null,
          created_by: req.user!.sub,
          consultation_fee: consultationFee,
        },
        include: {
